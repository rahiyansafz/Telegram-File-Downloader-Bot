...
using TelegramFileDownloaderBot;
...
{
    private const int
        _maxFileSize = 1000 * 1000 * 50,
        _maxTelegramSize = 20 * 1000 * 1000; // For some reasons, looks like there is some problems with 1024 * 1024 * 50 
    ...
    {
        Console.WriteLine("Downloader Bot Version " + _version);
        ...
        _freeBot = _config?.Admins is null || _config.Admins.Length == 0;
    ...
    private static async Task BotOnMessage(ITelegramBotClient bot, Update update)
    {
        ...
        if (size < _maxTelegramSize && Util.CheckExtenstion(Util.GetFileNameFromUrl(update.Message.Text)))
        {
            await bot.EditMessageTextAsync(update.Message.Chat, msg.MessageId,
        ...
        if (size < _config?.MaxFileSize) // Download the file, zip it and send it
        {
            ...
            if (size < _maxTelegramSize) // Send the file directly
            {
            ...
            else
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    zip.MaxOutputSegmentSize = _maxFileSize;
